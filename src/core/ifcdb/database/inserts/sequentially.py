...
class SeqInsert:
    ...
    uuid_map: dict[ifcopenshell.entity_instance, str] = field(default_factory=dict)

    def __post_init__(self):
        wrap = ifcopenshell.ifcopenshell_wrapper
        self.em = EdgeModel(schema=wrap.schema_by_name(self.ifc_schema))
    ...
    def create_insert_str(self):
        for i, item in enumerate(self.ifc_items, start=1):
            ...
            for j, att in enumerate(all_atts):
                att_str = get_att_insert_str(att, item, self.em, uuid_map=self.uuid_map, with_map=with_map)
                if j == len(all_atts) - 1:
                    comma_str = ""
                else:
                    comma_str = ",\n    "
    ...
    def insert(self, tx: edgedb.blocking_client):
        for item, insert_str in self.create_insert_str():
            ...
            self.uuid_map[item] = query_res["id"]

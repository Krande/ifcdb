...
class IfcDiff:
    ...
    def __init__(self, old_file, new_file, output_file=None, relationships=None, is_shallow=True, filter_elements=None):
    ...
    def export(self):
        with open(self.output_file, "w", encoding="utf-8") as diff_file:
            json.dump(self.to_json(), diff_file, indent=4)

    def load(self):
        print("Loading old file ...")
        self.old = self.old_file if isinstance(self.old_file, ifcopenshell.file) else ifcopenshell.open(self.old_file)
        print("Loading new file ...")
        self.new = self.new_file if isinstance(self.new_file, ifcopenshell.file) else ifcopenshell.open(self.new_file)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--relationships",
        type=str,
        help='A list of space-separated relationships, chosen from "type", '
        '"property", "container", "aggregate", "classification"',
        default="",
    )
